...
In this section we show an application of DPC to a real case study. The section organization is shown in Figure \ref{F:overview}. More precisely, we consider a real house located in L'Aquila, Italy, as a case study that we describe in Section \ref{SS:descriptionHouse}. Using historical data from this house, random forest models are built in Section \ref{SS:randomForestsModels} to be used either as plant or as predictive models for DPC. In Section \ref{SS:controllersDPCandBangbang}, DPC and bang-bang control strategies are defined to be used in the close-loop simulations. Finally in Section \ref{SS:simulationResults} controllers performance are compared in terms of energy saving using a thermal energy model of the house built via EnergyPlus in Section \ref{SS:energyPlusmodel}, using both historical data, and house construction layout and materials.

\begin{figure}[h!]
	\begin{center}
		\includegraphics[width=1\linewidth]{figures/overview.eps}
		\caption{Section \ref{S:realCaseStudy} organization scheme.}
