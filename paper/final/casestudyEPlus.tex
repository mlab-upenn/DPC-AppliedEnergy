...
\begin{figure}[h!]
	...
	\subfigure[Power tracking by DPC-En at 1.1 MW. In blue is the response when the optimal input is applied to the power Random Forest and in red is the response when the same input is applied to the plant (EnergyPlus building). The difference in closed-loop simulation and prediction is due to model mismatch.]{
		\label{F:eplus_power}
		\centering
