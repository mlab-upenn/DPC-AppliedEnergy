...
\begin{problem}\label{P:dpcRealCase}
	\begin{align}
		\begin{aligned}
		& \underset{u_{k+j},\epsilon_j^{min},\epsilon_j^{max}}{\text{minimize}} & & \sum_{j=1}^{N} Q{\tP}_{k+j}^2 + \lambda_{min}\parallel\epsilon^{min}\parallel_2 + \lambda_{max}\parallel\epsilon^{max}\parallel_2                                                                                                                \\
		& \text{subject to }                             & & \tP_{k+j}   = \hat{\Theta}_{\tP_j}   [ 1,u_k,\ldots,u_{k+j-1} ]^\top                                      \\
		...
		&                                                & & \underline{\tT}_{k+j-1}-\epsilon^{min}_j \leq \tT^3_{k+j} \leq \overline{\tT}_{k+j-1} + \epsilon^{max}_j  \\
