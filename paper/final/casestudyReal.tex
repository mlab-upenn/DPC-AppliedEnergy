\section{APPLICATION TO OPTIMAL HEATING SYSTEM SCHEDULING}
...
\textcolor[rgb]{0,0,1}{The contribution of this section, graphically shown in Figure \ref{F:overview}, is threefold.
\begin{enumerate}
...
\textcolor[rgb]{0,0,1}{In this section we study the robustness of our approach to noisy weather forecast. To this aim, we add noise with a zero-mean Gaussian distribution to the prediction variables $\tilde d(k+1),\ldots,\tilde d(k+N)$ used to obtain the parameters $\hat \Theta_{\cdot_j}$, $j=0,\ldots,N$ in the DPC Problem \eqref{E:DPCrealcase} (i.e.~the disturbance in input in the blue rectangle in Figure \ref{F:overview}), while we use the true values to simulate the process (i.e.~the disturbance in input in the box ``Plant" in the red rectangle in Figure \ref{F:overview}).\\
...
\textcolor[rgb]{0,0,1}{The following paragraphs provide comments on the change in performance due to inaccurate forecast on bounds violation and on energy consumption.\\
